            ...
            'fields' => static function () use ($registry, $productRepository): array {
                return [
                    'id' => [
                        ...
                        'resolve' => static fn (mixed $categoryValue): int =>
                            $categoryValue instanceof Category
                                ? $categoryValue->getId()
                                : (int) (is_array($categoryValue)
                                    ? ($categoryValue['id'] ?? 0)
                                    : 0),
                    ],
                    'name' => [
                        'type' => Type::nonNull(Type::string()),
                        'resolve' => static fn (mixed $categoryValue): string =>
                            $categoryValue instanceof Category
                                ? $categoryValue->getName()
                                : (string) (is_array($categoryValue)
                                    ? ($categoryValue['name'] ?? '')
                                    : ''),
                    ],
                    'products' => [
                        'type' => Type::nonNull(
                            Type::listOf(Type::nonNull($registry->product()))
                        ),
